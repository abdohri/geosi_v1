...
{
    public partial class RApdf : Form
    {
        String _idv;
        ...
        public void DoPdf(object sender, EventArgs e)
        {
            ...
            string repp = "select v.matricule,ha.localiisation ,  ha.date as Date_Debut, ha.duree ,DATEADD(MINUTE,ha.duree,ha.date) as dete_fin from  Historique_Arret ha "
...
+ " where cast(ha.date AS datetime) >='"+_dateDebut+"'  and  cast(ha.date AS datetime) <= '"+_dateFin+"' and ha.vehiculeid='" + _idv + "'";
            ...
            if (dat.Rows.Count > 0)
            {
                ...
                {
                    float[] widths = new float[] { 6f, 11f, 11f, 6f ,11f };
