...
var arrylistPointGPSN = new Array();
var mPolylinePointsSuivi2N = new Array;
var tabPoint = new Array();

var it = 0;
function TrajetSuiviN(mVar) {
    var objetInfo =
        {
    ...
    arrylistPointGPSN[objetInfo.vehiculeid] = objetInfo;

}
var t = 0;
function aff_veh(id) {
    carte.setCenter(arrylistPointGPSN[id].point);
  ...
  new google.maps.Point(10, 25));
    ...
    var mTempMaker = new google.maps.Marker({
        position: arrylistPointGPSN[id].point,
        map: carte,
        icon: mStartIcon,
        title: arrylistPointGPSN[id].matricule
    });

    google.maps.event.addListener(mTempMaker, 'click', function () {


        document.getElementById("matricule").innerHTML = arrylistPointGPSN[id].matricule;
        document.getElementById("code").innerHTML = arrylistPointGPSN[id].code;
        document.getElementById("vitesse").innerHTML = parseFloat(arrylistPointGPSN[id].speed) + " KM/H";
        document.getElementById("Date").innerHTML = arrylistPointGPSN[id].date;
        document.getElementById("Heure").innerHTML = arrylistPointGPSN[id].time;
        document.getElementById("adresse").innerHTML = arrylistPointGPSN[id].adresse;
        document.getElementById("nom").innerHTML = arrylistPointGPSN[id].nom;
        document.getElementById("prenom").innerHTML = arrylistPointGPSN[id].prenom;
        document.getElementById("permis").innerHTML = arrylistPointGPSN[id].permis;
        var img = "../../Ressources/Images/imagesupload/" + arrylistPointGPSN[id].imgVehicule;

        document.getElementById("imgVehicule").src = img;

        if (arrylistPointGPSN[id].contact == 0) {
            document.getElementById("imgContact").style.backgroundImage = 'url("../../Ressources/Images/contactOFF.png")';
        }
        else if (arrylistPointGPSN[id].contact == 1) {
            document.getElementById("imgContact").style.backgroundImage = 'url("../../Ressources/Images/contactON.png")';

        }

        if (arrylistPointGPSN[id].signialGPS < 10)
            document.getElementById("gps").style.backgroundImage = 'url("../../Ressources/Images/alerte1.png")';

        else if (arrylistPointGPSN[id].signialGPS < 80)
...
{
    alert(tabPoint.length);
    carte.setCenter(tabPoint[0].point);
    carte.setZoom(8);
    // alert(id)
    var mStartIcon = new google.maps.MarkerImage("../../Ressources/Images/V_bleu.png",
    new google.maps.Size(48, 27),
  ...
  new google.maps.Point(10, 25));
    ...
    for (i = 0; i < tabPoint.length; i++)
    {
        marker = new google.maps.Marker({
            position: tabPoint[i].point,
            map: carte,
            icon: mStartIcon,
            title: tabPoint[i].matricule
        });
...
function getpolygone(_poly) {
    return _poly;
}

function PlayAnimation(_poly) {
...
var finpoly;
var idpoly;
function getpolygone(_pd, _pf, _id) {
...
PolyAnimation = function (URLL) {
    ...
    $.ajax({
        ...
        data: { dateD: debutpoly, dateF: finpoly, id: idpoly },
        type: 'POST',
        beforeSend: function () {
...
function TrajetSuivi1(mVar) {

    var objetInfo = {
        matricule: mVar.matricule,
        typevehicule: mVar.typevehiculeid,
        code: mVar.code,
        date: mVar.date,
        time: mVar.time,
        contact: mVar.contact,
        speed: mVar.speed,
        point: new google.maps.LatLng(parseFloat(mVar.latitude), parseFloat(mVar.longitude)),
        adresse: getAdresse(parseFloat(mVar.latitude), parseFloat(mVar.longitude)),
        nom: mVar.nom,
        prenom: mVar.prenom,
        permis: mVar.permis,
        imgVehicule: mVar.imgVehicule,
        signialGPS: mVar.signialGPS
    ...
    if (arrylistPointGPS.length == 1) {
        ...
        for (i = 0; i < arrylistPointGPS.length; i++) {
            ...
            if (i == 0 && gPositions1[i].point != position) {
                ...
                mIconn = new google.maps.MarkerImage("../../Ressources/Images/iconetransparant.png",
                ...
                // var g = arrylistPointGPS[i];
                // alert("speed" + g.speed);
                // google.maps.event.addListener(mTempMaker, 'click', bulle(g)
                //{
                //alert("dfghjk,");
                //alert(g.speed);
                //infobulle(g.speed, g.point);
                //}
                //  );
...
function infobulle(speed, point) {
    alert("vbn,;");
    var infoWindow = new google.maps.InfoWindow({
        content: "Vitesse :" + speed + "Km/H  " + "Position  : (" + point + ")",
        position: point
    });
    infoWindow.open(carte, mTempMaker);
}
function bulle(g) {
    alert("dfghjk,");
    alert(g.speed);
